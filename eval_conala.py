...
path = os.path.dirname(os.path.abspath("eval_conala.py"))



model_checkpoints = [
    path + "/saved_models/codeparrot-small-conala-epoch-15", #0
...
tokenizer_checkpoint = path + "/saved_models/codeparrot-small-conala-epoch-15" 
tokenizer = AutoTokenizer.from_pretrained(tokenizer_checkpoint)
tokenizer.pad_token = tokenizer.eos_token


conala_dataset = load_dataset("AhmedSSoliman/CoNaLa") 
...
conala_tokenized_intents = conala_dataset.map(tokenize_intents, batched=True) 
conala_tokenized_intents = conala_tokenized_intents.remove_columns(["intent", "snippet"]) 
conala_tokenized_intents.set_format("torch")

conala_tokenized_snippets = conala_dataset.map(tokenize_snippets, batched=True) 
conala_tokenized_snippets = conala_tokenized_snippets.remove_columns(["intent", "snippet"]) 
conala_tokenized_snippets.set_format("torch")
...
conala_test_intents_dataloader = DataLoader(
    conala_tokenized_intents["test"],  batch_size=1, collate_fn=data_collator,
) 

conala_test_snippets_dataloader = DataLoader(
    conala_tokenized_snippets["test"],  batch_size=1, collate_fn=data_collator,
) 
...
for i, checkpoint in enumerate(model_checkpoints):
    ...
    f1 = open(path  + "/eval_conala-10s-" + str(i) + ".txt", "w")


    valid_progress_bar = tqdm(range(len(conala_test_intents_dataloader)))
    ...
    for batch_id, batch_intent in enumerate(conala_test_intents_dataloader):
        ...
        sample_outputs = model.generate(
            ...
            do_sample=True, 
            max_length= len(batch_intent["input_ids"][0])+len(conala_tokenized_snippets["test"][batch_id]["input_ids"]),
            top_k=50, 
            top_p=0.95,
            #num_beams = 20,
        ...
        real_snippet_text = tokenizer.decode(conala_tokenized_snippets["test"][batch_id]["input_ids"] , skip_special_tokens=True)
        ...
        for i, sample_output in enumerate(sample_outputs):
            ...
            references4 = conala_tokenized_snippets["test"][batch_id]["input_ids"].cpu().detach().numpy()
    ...
    avg_test_chrf = sum_test_chrf/len(conala_test_intents_dataloader)
    ...
    f1.write("count,count2,full_lenght: " + str(count) + " " + str(count2) + " " + str(len(conala_test_intents_dataloader)) + "\n")
