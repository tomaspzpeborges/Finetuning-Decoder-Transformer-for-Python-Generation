...
path = os.path.dirname(os.path.abspath("preprocess_and_train_django.py"))
...
model_checkpoints = [
    "/saved_models/codeparrot-small-conala-epoch-15", #intermediate models not available in gitlab
    #"/saved_models/codeparrot-small-conala-epoch-29",
    # "/saved_models/codeparrot-conala-epoch-15" 


model_checkpoint = path + model_checkpoints[0] 
...
raw_datasets = load_dataset('json', data_files={
    'train':base_url+ 'django_train.jsonl', 
    'validation': base_url + 'django_valid.jsonl',
})
...
# replace AdamW with Adafactor for memory reasons
optimizer = Adafactor(
    model.parameters(),
    lr=5e-4,
    eps=(1e-30, 1e-3),
    ...
    beta1=None,
    weight_decay=0.0,
    relative_step=False, #To use a manual (external) learning rate schedule you should set scale_parameter=False and relative_step=False.
...
output_dir = path + "/outputs/codeparrot-small-conala-epoch-15-django" 
